...
export class GameService {
  ...
  changePlayerTurn(squaredClicked: any){
    this.updateBoard(squaredClicked);
    if(!this.isGameOver) this.activePlayer = this.activePlayer==='X' ? 'O' : 'X';
    this.turnCount++;
    this.isGameOver = this.isGameOver ? true : false;
  }

  updateBoard(squaredClicked: any){
    this.board[squaredClicked.id].state = squaredClicked.state;
    if(this.isWinner){
      this.winner = true;
      this.isGameRunning = false;
